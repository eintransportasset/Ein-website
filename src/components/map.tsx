...
function GMap({ onLocationSelect, onBack }: GMapProps) {
  ...
  if (loadError || !googleMap_APIKEY) {
    return (
      <div className="flex items-center justify-center h-full border rounded-lg">
        <div className="text-center p-4 max-w-md">
          <h3 className="text-lg font-semibold text-red-600 mb-2">Failed to load Google Maps</h3>
          {!googleMap_APIKEY ? (
            <div className="bg-red-50 border border-red-200 rounded p-3 text-sm mb-3">
              <p className="font-medium text-red-800 mb-2">Missing API Key</p>
          ...
          {loadError && (
            <p className="text-xs text-gray-500 mt-2">Error: {loadError.message}</p>
          )}
        </div>
      </div>
    );
        ...
        >
          ...
          >
            <input
              ...
              id="auto-complete-input"
              placeholder=" Search Location"
              className="absolute z-10 w-11/12 px-4 pt-4 p-3 rounded-xl border border-gray-200 outline-none text-sm text-gray-800 shadow-lg bg-white/95 backdrop-blur-sm focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all duration-200"
            />
          </Autocomplete>
